...
<article id="bo_v" class="container margin-top-40" style="width:<?php echo $width; ?>">
    ...
    <section id="bo_v_atc">
        ...
        if($v_img_count) {
            ...
            /*
            기존 코드 
            배열 접근 시, 인덱스 번호는 0~n-1 번방 까지 존재하기에,
            조건식의 <= 기호 문제. ==> 수정
            isset 을 통해 배열 키 정의 확인 ==> 추가
        ...
        <!-- 본문 내용 시작 { -->
        <div>
			기업명 : <?php echo cut_str(get_text($view['wr_subject']), 70); ?>
		</div>
        <div>
			대표자명 : <?php echo $view['wr_1']?>
		</div>
        <div>
			회사 연락처 : <?php echo $view['wr_2']?>
		</div>
        <div">
			주소 : <?php echo $view['wr_3']?>
		</div>
    ...
    <div id="bo_v_share">
		...
		echo '<a href="#" class="btn_share btn btn_b03"><i class="fa fa-thumb-tack" aria-hidden="true"></i> SNS 공유</a>';
        ?>
    </div>
	-->
    <?php
