...
<article id="bo_v" class="container margin-top-40" style="width:<?php echo $width; ?>">
    <header>
        <h2 id="bo_v_title">
            ...
            <span class="bo_v_cate ks3"><?php echo $view['ca_name']; ?></span> 
            <?php } ?>
    ...
    <section id="bo_v_atc">
        ...
        if($v_img_count) {
            ...
            for ($i=0; $i<count($view['file']); $i++) {
                if (isset($view['file'][$i]) && $view['file'][$i]['view']) {
        ...
        <!-- 본문 내용 시작 { -->
        <div>기업명 : <?php echo cut_str(get_text($view['wr_subject']), 70); ?></div>
        <div>대표자명 : <?php echo $view['wr_1'] ?></div>
        <div>회사 연락처 : <?php echo $view['wr_2'] ?></div>
        <div>주소 : <?php echo $view['wr_3'] ?></div>
        <div>주력업종 : <?php echo $view['wr_4'] ?></div>
        <div>홈페이지 : 
            <?php if (!empty($view['wr_5'])) { ?>
            <a href="<?php echo get_text($view['wr_5']) ?>" target="_blank" rel="noopener"><?php echo get_text($view['wr_5']) ?></a>
            <?php } ?>
        </div>
        <!-- } 본문 내용 끝 -->
    ...
    <section id="bo_v_link">
        ...
        for ($i=1; $i<=count($view['link']); $i++) {
            ...
            <li>
                ...
                <a href="<?php echo $view['link_href'][$i] ?>" target="_blank"><strong><?php echo $link ?></strong></a>
...
$(function() {
    $("a.view_file_download").click(function() {
        if(!g5_is_member) { alert("다운로드 권한이 없습니다.\n회원이시라면 로그인 후 이용해 보십시오."); return false; }
        var msg = "파일을 다운로드 하시면 포인트가 차감(<?php echo number_format($board['bo_download_point']) ?>점)됩니다.\n\n포인트는 게시물당 한번만 차감되며 다음에 다시 다운로드 하셔도 중복하여 차감하지 않습니다.\n\n그래도 다운로드 하시겠습니까?";
        if(confirm(msg)) { var href = $(this).attr("href")+"&js=on"; $(this).attr("href", href); return true; } else { return false; }
    });
});
<?php } ?>
...
function excute_good(href, $el, $tx) {
    $.post(href, { js: "on" }, function(data) {
        ...
        if(data.count) {
            ...
            if($tx.attr("id").search("nogood") > -1) { $tx.text("이 글을 비추천하셨습니다."); }
            else { $tx.text("이 글을 추천하셨습니다."); }
